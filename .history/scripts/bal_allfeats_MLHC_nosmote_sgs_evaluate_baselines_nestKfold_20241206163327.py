"""
Driver to run binary classification models for specified heart failure (HF) endpoint. Prints results (performance, fairness, feature importance/coefficients) to specificed local MongoDB collection.
...
import shap
import imblearn
# TODO: make plots for logr coeffs.
# TODO: xgboost interpretability (Lime, shap etc): https://gust.dev/python/model-interpretability
...
# xgboost: tried sample wts: "scale_pos_weight": [1, 4]
...
"""
    ...
    "model": xgb.XGBClassifier(),
    "params": {
        "max_depth": [3, 6],  # Focus on shallow and medium complexity
        "n_estimators": [100, 500],  # A smaller range for boosting rounds
        "learning_rate": [0.01, 0.1],  # Two key learning rates to explore
        "subsample": [0.8],  # Fix at a common effective value
        "colsample_bytree": [0.8],  # Same for feature sampling
        "lambda": [1],  # Fixed value for L2 regularization
        "alpha": [0, 0.2],  # Minimal L1 regularization search
        "min_child_weight": [5],  # Fix at a reasonable default
        "gamma": [0],  # Ignore splits with penalties for now
        "eval_metric": ["logloss"],  # Same objective metric
    }
}
...
def main():
    ...
    # For Mongo server: parser.add_argument("-mongo_url", default = 'mongodb+srv://%s:%s@cluster0.ysobo3u.mongodb.net/' % (username, password)) 
    # For local Mongo:
