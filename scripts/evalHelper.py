...
# Fairness metrics: not imported since we modify the functions slightly.
...
"""
    Note IF fairness is low btw subgroups, 
    You may try adding a "fairness" penalty (e.g., eq odds, eq opportunity, disp impact)
"""
...
def evaluate_results(model, test_x, test_y, endpoint, imputer=None, scaler=None):
    ...
    # Note: Area under PR curve. May not need to also report APS.
    ...
    precision = skm.precision_score(
        test_y, binary_predictions
    )  # , zero_division=np.nan
    recall = skm.recall_score(
        test_y, binary_predictions
    )  # i.e., TPR, sensitivity, "equal opportunity". , zero_division=np.nan
